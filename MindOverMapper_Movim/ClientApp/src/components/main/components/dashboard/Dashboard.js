...
export default class Dashboard extends Component {
    ...
    setStateOnSaveChanges() {
        ...
        axios.put(`/api/project/${this.state.projectUID}`,
            {
    ...
    getProjectInfo = async (uid) => {
        ...
        if (response.areasOfResearch.length === 1) {
            ...
            r1l = (response.areasOfResearch[0].link.hrefName ? response.areasOfResearch[0].link.hrefName : '')
        }

        if (response.areasOfResearch.length === 2) {
            console.log("2");
            r1 = response.areasOfResearch[0].content
            r1l = response.areasOfResearch[0].link.hrefName
            r2 = response.areasOfResearch[1].content
            r2l = response.areasOfResearch[1].link.hrefName
        }
        ...
        if (response.areasOfResearch.length === 3) {
            ...
            r2 = response.areasOfResearch[1].content
            r2l = response.areasOfResearch[1].link.hrefName
            r3 = response.areasOfResearch[2].content
            r3l = response.areasOfResearch[2].link.hrefName
        }
                            ...
                            <div className='project-paper-holder'>
                                <Card style={{ height: 400 }}>
                                    ...
                                    <FontAwesomeIcon id='font-awesome-space' icon="share-square" />
                                                ...
                                                <Button id='learn-button' size="small" color="primary" onClick={() => this.handleOpenLearnModal(project.uid)}>
                                                    Learn More
                            ...
                            <Container id='tainer'>
                                ...
                                <Row>
                                    ...
                                    <Col md={{ span: 2, offset: 4 }}>
                                        <div id='save-changes'>
                                            <Button color='primary' onClick={() => { this.setStateOnSaveChanges() }}> Save Changes</Button>
                                        </div>
                                    </Col>
                                </Row>
                            </Container>
